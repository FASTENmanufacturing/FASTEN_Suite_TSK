...
//				} 
	...
	public void getCoordinates(RemoteStation remoteStation) {
		...
		String text = remoteStation.getCity().getName()+", "+remoteStation.getCity().getState().getName()+", Brazil";
		...
		try {
			...
			response.getFeatures().stream().forEach(f->{remoteStation.setLongitude(f.getGeometry().getCoordinates().get(0)); remoteStation.setLatitude(f.getGeometry().getCoordinates().get(1));});
		} catch (Exception e) {
			throw new BusinessException("Could not retrive Geolocation (lon, lat) for "+ remoteStation.getCity());
		}

		try {
			remoteStationControllerApi.updateRemoteStation(remoteStation.getId(), remoteStation);
		} catch (ApiException e) {
			throw new BusinessException("Could not save Geolocation (lon, lat) for "+ remoteStation.getCity());
		}	
