...
public class OperationalOptimizationListMB implements Serializable {
	...
	private void createHorizontalBarModel(List<OperationalOptimizationResult> results) {
        ...
        for (OperationalOptimizationResult operationalOptimizationResult : results) {
        	queueTime.set(operationalOptimizationResult.getRemoteStation().getName(), operationalOptimizationResult.getQueueTime());
        	timeProduction.set(operationalOptimizationResult.getRemoteStation().getName(), operationalOptimizationResult.getTimeProduction());
        	timeTravel.set(operationalOptimizationResult.getRemoteStation().getName(), operationalOptimizationResult.getTimeTravel());
		}
        Collections.reverse(results);
			...
			Call c = defaultApi.allocatePostAsync(entity.getCity().getName(), entity.getQuantity().toString(), entity.getPart().getName(),new ApiCallback<AllocationResult>() {
				...
				public void onSuccess(AllocationResult allocatePost, int statusCode, Map<String, List<String>> responseHeaders) {
					try {
						...
						for (AllocationResultInner allocation : allocatePost) {
							...
							operationalOptimizationResult.setRemoteStation(remoteStationControllerApi.retrieveRemoteStationByName(allocation.getRemoteStation()));
