...
public interface RemoteStationRepository extends JpaRepository<RemoteStation, Long>,  JpaLazyDataModel<RemoteStation>{
	...
	@Query("Select distinct(d.remoteStation) from Demand d where d.orderDate >= (select t.initialDate from TacticalOptimization t where t.id=:id) and d.orderDate <= (select t.endDate from TacticalOptimization t where t.id=:id)")
