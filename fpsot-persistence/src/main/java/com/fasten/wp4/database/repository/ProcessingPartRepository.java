...
public interface ProcessingPartRepository extends JpaRepository<ProcessingPart, Long>, JpaLazyDataModel<ProcessingPart>, CustomizedProcessingPartRepository{
	...
	@Query("SELECT ceil(coalesce(max( (processingPart.averagePrintTime) ), 0)/3600) FROM ProcessingPart processingPart")
	Long retrieveMaxHoursProcessing();

	@Query("select max((processingPart.averagePrintTime)*d.quantity)/3600"
