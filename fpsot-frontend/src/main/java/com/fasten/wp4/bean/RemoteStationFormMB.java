...
public class RemoteStationFormMB implements Serializable {
	...
	public void populateSelectCity() {
		if(has(remoteStation.getAddress().getState())) {
			try {
				cities= cityControllerApi.retrivedByState(remoteStation.getAddress().getState().getId());
			} catch (ApiException e) {
	...
	public List<City> getCities() {
		if(cities==null) {
			...
			return cities;
		}else if(cities.isEmpty() && has(remoteStation.getAddress().getState())) {
			populateSelectCity();
		}
		return cities;
	}
