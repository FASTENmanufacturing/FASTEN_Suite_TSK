package com.fasten.wp4.optimizator.tactical.cplex;
...
public class PMedian {
	...
	public PMedian(TacticalOptimization tacticalOptimization, int[] distributionCenters,
		...
		this.horizon=horizon;										//the horizon to accomplish the demands in days
	}

	//the full analysis varying p from one to one on each warehouse candidates
	public TacticalOptimizationResponse analysis() {
		
		DecimalFormat df = new DecimalFormat();
		df.setMaximumFractionDigits(2);
		
		//first solution avaiable
		TacticalOptimizationResponse analysis = new TacticalOptimizationResponse();
		...
		TacticalOptimizationResult costBenefit=null;
		
		//vary p to get first solution
		for (int p = 1; p <= remoteStations.length; p++) {
			
			TacticalOptimizationResponse tacticalOptimizationResponse = solve(p);
			CplexResult cplexResult = tacticalOptimizationResponse.getCplexResult();
			TacticalOptimizationResult tacticalOptimizationResult = tacticalOptimizationResponse.getTacticalOptimizationResult();

			if(cplexResult.getObjectiveStatus().equals(Status.Optimal)) {
				firstSolutionP=p;
				maxResult=cplexResult.getObjectiveValue();
				
				tacticalOptimizationResult.setObjectivePercent(100d);
				tacticalOptimizationResult.setpPercent(((double)(p)/(remoteStations.length))*100);
				tacticalOptimizationResult.setCostBenefit(false);
				
				if(LOGGER.isDebugEnabled()) {
					LOGGER.debug(	
							" percent p: "+df.format(tacticalOptimizationResult.getpPercent())+"% percent " +
							"obj: "+df.format(tacticalOptimizationResult.getObjectivePercent())+"%"
					);
				}
				
				analysis.addTacticalOptimizationResults(tacticalOptimizationResult);				
				break;
			}else {
				tacticalOptimizationResult.setUnfeasible(true);
				analysis.addTacticalOptimizationResults(tacticalOptimizationResult);
			}
		}
		
		//vary p from first solution to all range of warehouses candidates
		for (int p = firstSolutionP+1; p <= remoteStations.length; p++) {
			
			TacticalOptimizationResponse tacticalOptimizationResponse = solve(p);
			CplexResult cplexResult = tacticalOptimizationResponse.getCplexResult();
			TacticalOptimizationResult tacticalOptimizationResult = tacticalOptimizationResponse.getTacticalOptimizationResult();
			
			tacticalOptimizationResult.setObjectivePercent((cplexResult.getObjectiveValue()/maxResult)*100);
			tacticalOptimizationResult.setpPercent(((double)(p)/(remoteStations.length))*100);
			tacticalOptimizationResult.setCostBenefit(false);
			
			if(costBenefit==null && (tacticalOptimizationResult.getObjectivePercent()<tacticalOptimizationResult.getpPercent())) {
				tacticalOptimizationResult.setCostBenefit(true);
				costBenefit=tacticalOptimizationResult;
			}
			if(LOGGER.isDebugEnabled()) {
				LOGGER.debug(	
						" percent p: "+df.format(tacticalOptimizationResult.getpPercent())+"% percent " +
	...
	public TacticalOptimizationResponse costBenefit(){
		
		DecimalFormat df = new DecimalFormat();
		df.setMaximumFractionDigits(2);
		
		//first solution avaiable
		TacticalOptimizationResponse response = new TacticalOptimizationResponse();
		Double maxResult = null;		//the worst optimal solution will occur at the first feasible solution
		Integer firstSolutionP=null;	//the fist feasible P
		TacticalOptimizationResult costBenefit=null;
		...
		for (int p = 1; p <= remoteStations.length; p++) {
			...
			if(cplexResult.getObjectiveStatus().equals(Status.Optimal)) {
				...
				//response.addTacticalOptimizationResults(tacticalOptimizationResult);				
				break;
			}else {
				tacticalOptimizationResult.setUnfeasible(true);
				//response.addTacticalOptimizationResults(tacticalOptimizationResult);
		...
		for (int p = firstSolutionP+1; p <= remoteStations.length; p++) {
			...
			response.setTacticalOptimizationResult(tacticalOptimizationResult);
		}
	 ...
	 */
	public TacticalOptimizationResponse solve(int p) {
		...
		try {
			...
			if (cplex.solve()) {
				...
				//Generic
				CplexResult cplexResult = new CplexResult();
				...
				cplexResult.setObjectiveStatus(Status.valueOf(cplex.getStatus().toString()));
				cplexResult.setObjectiveValue(cplex.getObjValue());
				...
				cplexResult.setVariables(variables);
				...
				cplexResult.setConstraints(constraints);
				
				return new TacticalOptimizationResponse(cplexResult, tacticalOptimizationResult); 
				...
				TacticalOptimizationResult unfeasible = new TacticalOptimizationResult();
				unfeasible.setUnfeasible(true);
				return new TacticalOptimizationResponse(unsolved, unfeasible);
			}

		}catch(IloException e) {
			CplexResult error = new CplexResult();
			error.setObjectiveStatus(Status.Error);
			return new TacticalOptimizationResponse(error, null);
		} finally {
