...
public class PMedianPartsPrintersComplexTskMatrix {
	...
	public PMedianPartsPrintersComplexTskMatrix(TacticalOptimization tacticalOptimization, int[] distributionCenters,
	...
	public HashMap<String, Object> solve(int p) {
		...
		try {
				...
				for (int w = 0; w < m; w++) {
					for (int s = 0; s < k; s++) {
						if( (numberOfOrder[c][s]!=0) && (demandByPart[c][s]!=0) ) {
							...
							objective.addTerm(processingCost[c][w][s],shipToCustomer[c][w][s]); // this term became Zero
						}
					}
				}
			}
			for (int w = 0; w < m; w++) {
				objective.addTerm(-1.0*totalTimeToExecuteOrders,sramsInWarehouse[w]); //this term allocate SRAMs and eliminates the second term 
			}
			...
			for (int w=0; w<m; w++) {
				...
				IloRange capacityConstraint = cplex.addGe(capacityConstraintsExpression[w], 0.0);
				capacityConstraint.setName("maxSramsConstraint"+"_"+warehouses[w]);
				capacityConstraints[w]=capacityConstraint;
			}

			//------------------------ maxSramConstraint ------------------------
			//the maximum SRAMs allocated to the Supply Chain must be the total need to meet all demand
			IloLinearNumExpr maxSramConstraintExpression = cplex.linearNumExpr();
			for (int w=0; w<m; w++) {
				maxSramConstraintExpression.addTerm(1.0, sramsInWarehouse[w]);
			}
			IloRange maxSramConstraint = cplex.addLe(maxSramConstraintExpression, maxSrams);
			maxSramConstraint.setName("maxSramConstraint");

			//------------------------ allocateSramsConstraints ------------------------
			//only allow allocations to the chosen place as open warehouse
			IloConstraint[] allocateSramsConstraints = new IloConstraint[m];
			IloLinearNumExpr[] allocateSramsConstraintExpressions = new IloLinearNumExpr[m];
			for (int w=0; w<m; w++) {
				allocateSramsConstraintExpressions[w] = cplex.linearNumExpr();
				allocateSramsConstraintExpressions[w].addTerm(1.0, sramsInWarehouse[w]);
				allocateSramsConstraintExpressions[w].addTerm(-1.0*maxSrams, openWarehouse[w]);
				IloRange allocateSramsConstraint = cplex.addLe(allocateSramsConstraintExpressions[w], 0.0);
				allocateSramsConstraint.setName("allocateSramsConstraint"+"_"+warehouses[w]);
				allocateSramsConstraints[w]=allocateSramsConstraint;
			}
			...
			if (cplex.solve()) {
				...
				if(LOGGER.isTraceEnabled()) {
					...
					String[] maxSramConstraintLog = new String[]{((IloRange) maxSramConstraint).toString().replace("IloRange ","")+" -> slack = "+cplex.getSlack((IloRange) maxSramConstraint)};
					LOGGER.trace("maxSramConstraint = \n"+PrettyPrintMatrix.printStr(maxSramConstraintLog));

					String[] allocateSramsConstraintsLog = new String[m];
					for (int w=0; w<m; w++) {
						allocateSramsConstraintsLog[w]=((IloRange) allocateSramsConstraints[w]).toString().replace("IloRange ","")+" -> slack = "+cplex.getSlack((IloRange) allocateSramsConstraints[w]);
					}
					LOGGER.trace("allocateSramsConstraints = \n"+PrettyPrintMatrix.printStr(allocateSramsConstraintsLog));
				...
				Constraint maxSramConstraintResult = new Constraint();
				maxSramConstraintResult.setFunction((maxSramConstraint).toString());
				maxSramConstraintResult.setSlack(cplex.getSlack((IloRange) maxSramConstraint));
				constraints.add(maxSramConstraintResult);

				for (int w = 0; w < m; w++) {
					Constraint constraint = new Constraint();
					constraint.setFunction(((IloRange) allocateSramsConstraints[w]).toString());
					constraint.setSlack(cplex.getSlack((IloRange) allocateSramsConstraints[w]));
					constraints.add(constraint);
				}
